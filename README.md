...
| 模块 | 说明 | 状态 |
|---|---|---|
| **拓词** | 客户原始词 + 客户网址 + 竞品网址 + 客户资料 → 一键：Google 关键词规划师拓词 → DeepSeek 生成筛词清单（勾选采用，重打分不取数）→ 打分（口径规则，零 AI）→ DeepSeek 布词（抓站点 URL，一页一主词，出「URL布词视图」）→ 五表 xlsx，对齐飞书总表 | ✅ 可用 |
| **排名监控** | DataForSEO 拉 Google 自然结果，SQLite 留底，队列模式约 $0.0015/词 | ✅ 可用 |
| **GSC 周报** | Search Console 近 7 天 vs 前 7 天，按《SEO 客户周同步》模板出稿；判定（正常 / 关注 / 异常 / 背离）和下钻 Top 由规则算，AI 只写结论 / 三段式解读 / 本周计划；建飞书文档并私聊链接 | ✅ 可用 |
...
**飞书词库是唯一源头。** 选了项目（项目里配了 `feishu_url`）会自动去飞书拉一次，
不再用 `config.local.yaml` 里那份手抄副本 —— 两份并存会让人分不清
手上这批词到底是哪来的。

文本框上方常驻一行来源标记：`来自飞书词库 · 66 词 · 14:32 同步` /
`手动填写` / `来自配置文件（不是飞书）`。手改文本框，来源立刻变「手动」。
扣费确认框里也会带上这一行，作业日志里也记一条。

> ⚠️ **配了飞书链接却用了非飞书的词，会红字警告。** 这是最容易悄悄出错的地方：
> 飞书表这个月加了词，你没重新拉，就会拿上个月的词表去查 —— 花了钱、新词永远没有排名，
> 而且结果还会写回飞书表。旧版本对此毫无提示。

拉失败时文本框保持空，**不会回落到配置里的旧词** —— 宁可没词，
也不能让人以为手上这批是刚从飞书拿的。
