...
| 模块 | 说明 | 状态 |
|---|---|---|
| **拓词** | 统一走 Google 关键词规划师（Google Ads API，不用 Semrush）：客户原始词（原样进表 + 兼作种子拓展，可关）/ 竞品网址两路词源可叠加，合并去重后两轮取数，按 20 列 SOP 总表出四表 xlsx；意图 / 优先级 / 金矿按口径规则判定，零 AI；DeepSeek 只负责从客户资料生成筛词清单 | ✅ 可用 |
