...
TEMPLATES = {
    ...
    "customer": {
        ...
        "notes": [
            ...
            "客户中文和客户级别会进「2.原始词核对」页，客户级别同时带进总表；",
            "这些词在总表里词源标成「原始词」。",
            "",
            "⚠ 客户级别不参与优先级判定。实测两者不相关 —— 客户按「产品重要性」打分，",
            "SOP 按「搜索价值」定级，是两把尺子。它的用处是你定布局角色和目标 URL 时参考。",
            "",
            "不想改格式的话，**客户原表可以直接传上来**，这几种情况都能自动处理：",
            "  · 第一列是序号 → 自动跳过",
            "  · 带表头（序号 / 关键词 / 中文 / 级别）→ 自动剥掉",
            "  · 中文列有合并单元格 → 空着的那行级别不会串位",
            "也可以在文本框里直接粘，用 Tab 或逗号分隔即可。",
        ],
    },
    "mixed": {
        "title": "混杂泛词清单",
